...
public class WidgetHandler implements com.zc.cliq.interfaces.WidgetHandler {
	...
	public WidgetResponse viewHandler(WidgetExecutionHandlerRequest req) throws Exception {
		...
		if (req.getEvent().equals(WIDGET_EVENT.LOAD) || (req.getEvent().equals(WIDGET_EVENT.TAB_CLICK) && req.getTarget().getId().equals("leaderboardTab"))) {
			...
			// Table headers
			table.setHeaders(Arrays.asList("Username", "Score", "Rank"));

			List<HashMap<String, String>> data = new ArrayList<>();
			for (int i = 1; i <= 30; i++) {
				HashMap<String, String> row = new HashMap<>();
				row.put("Username", "Player " + i);
				row.put("Score", String.valueOf(500 - i * 10));
				row.put("Rank", (i) + " " + getRankEmoji(i));
				data.add(row);
			}
			table.setRows(data);
			...
			if (target.equals("history")) {
				...
				// Table headers for Quiz History
				historyTable.setHeaders(Arrays.asList("Quiz Name", "Date", "Score"));

				// Sample Data for Quiz History
				List<HashMap<String, String>> historyData = new ArrayList<>();
				for (int i = 1; i <= 10; i++) {
					HashMap<String, String> row = new HashMap<>();
					row.put("Quiz Name", "Quiz " + i);
					row.put("Date", "2024-11-" + (i < 10 ? "0" : "") + i);
					row.put("Score", String.valueOf(100 - i * 5));
					historyData.add(row);
				}
				historyTable.setRows(historyData);
